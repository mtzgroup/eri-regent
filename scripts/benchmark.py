...
def plot_timings(num_molecules, timings, title, file):
    ...
    plt.ylabel("Runtime")
    plt.errorbar(num_molecules, np.mean(timings, axis=1), fmt="ko",
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_cpus', default=1, type=int, help='The number of compute CPUs to use.')
    parser.add_argument('--num_gpus', default=1, type=int, help='The number of GPUs to use.')
    parser.add_argument('--num_trials', default=1, type=int, help='Repeat each test NUM_TRIALS times.')
    parser.add_argument('--savefig_file', default=None, metavar='FILE', help='Save figure to FILE.')
    args = parser.parse_args()

    # TODO: Iterate over number of gpus
    # TODO: Pickle results to plot later
    print("Running with %d CPUs and %d GPUs for %d trials" % (args.num_cpus, args.num_gpus, args.num_trials))
    ...
    if args.savefig_file is not None:
        plot_timings(num_molecules, timings,

...
function jfock(r_jbras_list, r_jkets_list, r_gamma_table, threshold, parallelism, launch_jgrad)
      ...
      for L3 = 0, getCompiledMaxMomentum() do -- inclusive
        for L4 = L3, getCompiledMaxMomentum() do -- inclusive
          ...
          if use_mcmurchie then
            local jfock_integral
            if launch_jgrad then
              jfock_integral = generateTaskMcMurchieJFockIntegral(L12 + 1, L34)
            else
              jfock_integral = generateTaskMcMurchieJFockIntegral(L12, L34)
            end

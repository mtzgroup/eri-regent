-- Assumes fspaces and `computeR0004` have been previously declared.
import "regent"
...
__demand(__leaf)
__demand(__cuda)
task coulombPPPP(r_bra_gausses : region(ispace(int1d), HermiteGaussian),
...
do
  for bra_idx in r_bra_gausses.ispace do
    for ket_idx in r_ket_gausses.ispace do
      ...
      var R000 : double[5] = __demand(__inline, computeR0004(t, alpha, r_boys))
      ...
      r_j_values[bra.data_rect.lo + 4].value += result[4]
      r_j_values[bra.data_rect.lo + 5].value += result[5]
      r_j_values[bra.data_rect.lo + 6].value += result[6]
      r_j_values[bra.data_rect.lo + 7].value += result[7]
      r_j_values[bra.data_rect.lo + 8].value += result[8]
      r_j_values[bra.data_rect.lo + 9].value += result[9]
    end
  end
end
